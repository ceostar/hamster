...
def login():
	try:
		...
		try:
			sy = requests.get('https://graph.facebook.com/me?fields=name,id&access_token={token}'+tokenku[0], cookies={'cookie':cok})
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tSaran Extension : [green]CookieDough[white]',width=90))
		ses = requests.Session()
		cookie = input('\nMasukan Cookie : ')
